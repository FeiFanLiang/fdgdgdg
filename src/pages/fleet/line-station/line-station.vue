...
<div id="lineStation">
    <el-row :gutter="20">
     ...
     <el-table :data="list" ref="table" style="width: 100%" element-loading-text="拼命加载中" v-loading="loading" border>
      <el-table-column prop="CarLineID" label="路线ID" show-overflow-tooltip width="150"></el-table-column>
      <el-table-column prop="CarLineName" label="路线名称" show-overflow-tooltip width="150"></el-table-column> 
    ...
    <el-dialog :title="dialogTitle" v-model="showAddDialog" size="tiny" @close="resetForm('form')">
      <el-form ref="form" :model="form">
        ...
        <el-form-item label="站点名称" prop="StationID">
            <el-select v-model="form.StationID" clearable placeholder="请选择站点名称" style="width:70%">
        ...
        <el-form-item label="站点次序" prop="Order">
            <el-input type="number" placeholder="请输入第几站" v-model="form.Order" style="width:70%"></el-input>
        </el-form-item> 
      </el-form>
  ...
  methods: {
      async fetchData() {
        ...
        const options = {
         ...
         try { 
          const res = await lineStationApi.listByQuery();
      ...
      async carlineChange(value){
          ...
          let a = res.data.Data
          this.form.StationID= ''
          for(let s in this.stationList){
            delete this.stationList[s].disabled;
          }
          for(let i in a){
            if(value == a[i].CarLineID){
                for(let j in this.stationList){
                    if(a[i].StationID == this.stationList[j].ID){
                        this.stationList[j].disabled= true
                    }
                }
            }
          }
