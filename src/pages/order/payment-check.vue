    ...
    <el-table :data="paymentCheck" style="width: 100%" border element-loading-text="拼命加载中" v-loading="loading" @expand="expand" row-key="ID" :expand-row-keys="expandRowKeys">
        <el-table-column type="expand" width=25>
            <template scope="props">
                <!-- <el-table :data="expandlist" border style="width: 100%">
                    <el-table-column label="款项金额" prop="AmountUse"></el-table-column>
                    <el-table-column label="款项类别" prop="PaymentTag">
                        <template scope="scope">
                            <span v-if="scope.row.PaymentTag === 0">收款</span>
                            <span v-if="scope.row.PaymentTag === 1">付款</span>
                        </template>
                    </el-table-column>
...
<script>
import { paymentCheckApi } from 'api'
export default {
  data(){
      return{
          currentPage: 1,
  ...
  methods:{
    ...
    async expand(row, expanded){
        const _self = this
        if (expanded) {
            const res = await paymentCheckApi.listOrder(row.ID)
            _self.expandlist = res.data.Data
