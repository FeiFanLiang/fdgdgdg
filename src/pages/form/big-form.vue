...
export default {
    ...
    methods: {
        async getSchoolList() {
            const data = await oldApi.fetchSchoolList()
            let {
                code,
                school_list
            } = data;
            if (code === 200) {
                school_list.forEach(school => {
                    ...
                    school['major_list'].forEach(major => {
        ...
        async getWorkDurationOptions() {
            const data = await oldApi.fetchWorkDurationOptions()
            let {
                code,
                workDurationOptions
            } = data;
        ...
        async getAcademicOptions() {
            const data = await oldApi.fetchAcademicOptions()
            let {
                code,
                academicOptions
            } = data;
        ...
        handleError() {},
        async uploadResume() {
            const data = await oldApi.postResume({
                data: this.resume[0]
            })
            let {
                code,
        ...
        async submit() {
            ...
            if (!valid) {
                ...
                await oldApi.createResume(this.form)
                this.$message({
                    message: '提交成功',
                    type: 'success'
                });
