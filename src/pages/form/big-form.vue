...
export default {
  ...
  methods: {
    ...
    getSchoolList() {
      oldApi.fetchSchoolList().then(data => {
    ...
    getWorkDurationOptions() {
      oldApi.fetchWorkDurationOptions().then(data => {
    ...
    getAcademicOptions() {
      oldApi.fetchAcademicOptions().then(data => {
    ...
    uploadResume() {
      oldApi.postResume({data: this.resume[0]}).then(data => {
    ...
    submit() {
      this.$refs.form.validate(valid => {
        if (!valid) {
          ...
          oldApi.createResume(this.form).then(() => {
            this.$message({
              message: '提交成功',
              type: 'success'
            });
