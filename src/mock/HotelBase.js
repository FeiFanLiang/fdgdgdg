...
const API = {
  listAll(config) {
    let { id, HotelName, HotelName_En, page } = config.params;
    let mockHotelbase = _hotelbase.filter(hotelbases => {
      if (HotelName && hotelbases.HotelName !== HotelName) return false;
      if (HotelName_En && hotelbases.HotelName_En !== HotelName_En)
        return false;
      if (id && hotelbases.id !== id) return false;
      return true;
    });
    if (page === 0) page++;
    mockHotelbase = mockHotelbase.filter(
